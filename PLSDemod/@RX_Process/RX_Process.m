...
%         symb_pattern = []
    ...
    methods   %constructor method
        function obj=ConstructorFunc(varargin)
            ...
            obj.bit_codebook = varargin{6};
            obj.num_data_bins = varargin{5};
        ...
        function chan_est_bins_sort=ChannelEstimate(obj, varargin)
            obj = varargin{1};
            obj.buffer_rx_data = varargin{2};
            obj.ref_signal = varargin{3};
            ...
            for symb = 1:(obj.num_data_symb)
                ...
                for ant = 1:obj.num_ant
                    time_data = obj.buffer_rx_data(ant, symb_start:symb_end);
                    data_fft = fft(time_data, obj.NFFT);
                    data_in_used_bins = data_fft(obj.used_data_bins);

                    est_channel = data_in_used_bins * (conj(obj.ref_signal(symb, :))/(abs(obj.ref_signal(symb, :))))';
